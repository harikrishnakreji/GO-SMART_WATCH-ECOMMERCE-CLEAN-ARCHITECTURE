...
func (o *orderRepository) DoesCartsExist(userID int) (bool, error) {
...
func (o *orderRepository) AddOrderItems(orderItemDetails domain.OrderItem, UserID int, ProductID uint, Quantity float64) error {
	...
	// after creating the order delete all carts items and also update the quantity of the product
...
func (o *orderRepository) GetOrderDetails(userID int, page int, count int) ([]models.FullOrderDetails, error) {
	...
	for _, od := range orderDetails {
		...
		o.DB.Raw("select order_items.product_id,products.name,order_items.quantity,order_items.total_price from order_items inner join products on order_items.product_id = products.id where order_items.order_id = ?", od.OrderId).Scan(&orderProductDetails)
